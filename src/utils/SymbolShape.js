...
export class SymbolShape extends BaseShape {
  constructor() {
    super();
    this.pt = new Point(0, 0);
    this.type = SHAPE_Type.SYMBOL;
    this.id = 0;
  ...
  load() {
    var result = BaseShape.svgImages[this.id + 1].slice();
  ...
  static from(json) {
    var symbolShape = new SymbolShape();
    symbolShape.type = json.type;
    symbolShape.color = json.color;
    symbolShape.pt.x = json.pt.x;
    symbolShape.pt.y = json.pt.y;
    symbolShape.id = json.id;
    symbolShape.load();
    symbolShape.boundRect.leftTop = new Point(
      json.boundRect.leftTop.x,
      json.boundRect.leftTop.y
    );
    symbolShape.boundRect.rightBottom = new Point(
  ...
  contains(pt) {
    ...
    if (rect.contains(pt, BaseShape.penWidth)) return 0;
    return -1;
  }
  intersects(pt1, pt2) {
